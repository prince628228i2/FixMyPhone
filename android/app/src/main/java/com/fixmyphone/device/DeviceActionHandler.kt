...
class DeviceActionHandler(private val ctx:Context):ActionHandler{
 override fun canHandle(a:ActionType)=a in setOf(ActionType.CALL,ActionType.SPEAKER_ON,ActionType.SPEAKER_OFF,ActionType.OPEN_APP,ActionType.SET_VOLUME,ActionType.TOGGLE_WIFI,ActionType.TOGGLE_BLUETOOTH,ActionType.TOGGLE_AIRPLANE_MODE,ActionType.MEDIA_PLAY,ActionType.MEDIA_PAUSE,ActionType.MEDIA_SEEK,ActionType.RESOLVE_CONTACT)
 ...
 ActionType.TOGGLE_WIFI->{if(android.os.Build.VERSION.SDK_INT>=29){ctx.startActivity(Intent(Settings.ACTION_WIFI_SETTINGS).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK));return "requires_user"}}
